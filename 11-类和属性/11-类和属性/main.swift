...
var r2 = r1
r2.show()
// 类是引用类型, 类之间的赋值其实是将r2指向了r1的存储控件, 所以他们是两个指向同一块存储空间, 修改其中一个会影响到另外一个
r1.width = 99
r1.show()
r2.show()
...
var p:Person = Person(name: "gaojun", age: 24)
p.name = "hm"
p.age = 50
...
var p2: Person2 = Person2(name: "gaojun", age: 24, card: "411481199204108489")
p2.name = "hm"
p2.age = 50
// 构造好对象之后不能修改常量存储属性
//p2.card = "56789"
...
// 不能修改[结构体/枚举常量]对象中的值, 因为他指向的对象是一个常量
//p3.name = "hm"
// 不能修改结构体常量对象的值
...
/*
延迟存储属性
Swift语言中所有的存储属性必须有初始值, 也就是当构造完一个对象后, 对象中所有的存储属性必须有初始值, 但是也有例外, 其中[延迟存储属性]可以将属性的初始化推迟到该属性第一次被调用的时候
懒加载应用场景:
1.有可能不会用到
2.依赖于其它值
*/
